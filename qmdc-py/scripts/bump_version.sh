...
echo "Updating qmdc/__init__.py..."
sed -i.bak "s/__version__ = \"$CURRENT_VERSION\"/__version__ = \"$NEW_VERSION\"/" qmdc/__init__.py
rm qmdc/__init__.py.bak
...
echo "  2. Commit: git add -u && git commit -m 'Bump Python parser version to $NEW_VERSION'"
echo "  3. Tag: git tag py-v$NEW_VERSION"
