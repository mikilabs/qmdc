...
def _is_id_query(query: str) -> bool:
    ...
    words = query.split()
    if len(words) <= 2:
        # Check for patterns like QMD-17, QMDC17, TASK-123
        for word in words:
...
def graph_walk(
    ...
    Uses both explicit edges (from qmdc_parser) and inferred edges.
...
def _object_in_excluded_ns(object_id: str, storage: Storage, exclude_ns: list[str]) -> bool:
    ...
    Args:
        object_id: Object's global ID (e.g., "docs:tracking:qmd55").
