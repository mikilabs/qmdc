"""Configuration loading for QMD Semantic."""
...
def load_config(workspace_path: Path | str | None = None) -> Config:
    ...
    Args:
        workspace_path: Path to workspace directory. If provided, looks for
            .qmd-semantic/config.yaml in that directory first.
    ...
    # 1. Global config (~/.qmd-semantic/config.yaml)
    global_config = Path.home() / ".qmd-semantic" / "config.yaml"
    if global_config.exists():
        with open(global_config) as f:
            global_data = yaml.safe_load(f) or {}
            config_data.update(global_data)

    # 2. Workspace config (.qmd-semantic/config.yaml)
    if workspace_path:
        workspace_path = Path(workspace_path)
        workspace_config = workspace_path / ".qmd-semantic" / "config.yaml"
